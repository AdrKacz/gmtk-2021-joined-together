...
void fragment() {
	...
	for (int y = -1 ; y <= 1 ; y++) {
		for (int  x = -1 ; x <= 1 ; x++) {
			...
			float star = star(gv - offs - vec2(n, fract(n * 34.0)) + 0.5, smoothstep(0.8, 0.9, size), 0.07 +  0.02 * sin(fract(n *1234.0) * 6.28 + TIME * 5.0)) / 9.0;
...
//	float black = distance(uv_scaled, vec2(0.0));
//	color -= black;
